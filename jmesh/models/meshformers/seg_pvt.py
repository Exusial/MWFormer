...
def online_process(feats, res_faces, adjdict, patch_num, center, next_size):
    N, F, _ = feats.shape
    # adjlist = []
    ...
    patches = patch_aggregation("add", feats, res_mat, patch_num)
    ...
    return patches, res_mat, centers, adjdict

def online_process_k(feats, center, patch_num, res_faces, oriadjlist, k, euc=False, Fs=None):
    N, F, _ = feats.shape
    init_faces = get_patches_ffs(center, F, patch_num, Fs=Fs)
    # adjdict = get_hierar_FAF_cpu(patch_num, oriadjlist, res_mat, next_size)
    if euc:
        # scenter = center.reindex([N, int(patch_num), 3], ['i0', '@e0(i0, i1)', 'i2'], extras=[init_faces])
        kset = knn_indices_func_gpu(center, center, k, 1)
    else:
        kset = k_bfs_patch_cpu(oriadjlist, F, k, False)
    return None, kset, kset.reindex([N, int(patch_num), k], ['i0', '@e0(i0, i1)', 'i2'], extras=[init_faces]), init_faces

class PvTSegTransformer(nn.Module):
    def __init__(self,
                 num_patches=[128],
                 num_classes=[30], 
                 embed_dim=128, 
                 ...
                 down=1,
                 merge_batch=1):
        super(PvTSegTransformer,self).__init__()
        self.num_patches = copy.deepcopy(num_patches)
        self.num_patches.append(self.num_patches[-1])
        self.sr_ratio = sr_ratio
        self.patch_size = patch_size
        self.down = down
        ...
        self.transformers = nn.ModuleList([
            PatchHieTransformer(
                num_patches=num_patches[i]*merge_batch, num_classes=num_classes[i], embed_dim=embed_dims[i], depth=depth[i], attn_drop_rate=attn_drop_rate, drop_rate=drop_rate,                 drop_path_rate=drop_path, sr_ratio=sr_ratio[i]
            ) for i in range(len(num_patches))
        ])
        self.fc = nn.Linear(num_classes[-2], num_classes[-1])
        self.convs = nn.ModuleList([
            SubConv(num_classes[i], num_classes[i]) for i in range(len(num_patches))
        ])
        # self.transformers.append(
        #     PatchTransformer(
        #         num_patches=num_patches[-1], num_classes=num_classes[-1], embed_dim=embed_dims[-1], depth=depth, attn_drop_rate=attn_drop_rate, drop_rate=drop_rate,                  drop_path_rate=drop_path
        #     )
        # )

    def execute(self, x, res_faces, adjdict, k, feats, ksets, center, euc=False, Fs=None):
        res_faces_list = []
        ...
        for idx, transformer in enumerate(self.transformers):
            ...
            _, kset, tkset, init_faces = online_process_k(x, center, self.num_patches[idx] * self.merge_batch // self.sr_ratio[idx], res_faces, adjdict, k, euc, Fs=Fs)
            ksets.append(kset)
            # print(f"do {idx} transformer")
            # print(x.shape, f"{idx}")
            x = transformer(x, k, tkset, init_faces) # (N, patch_num, F)
            # print("append x:", x.shape)
            feats.append(x)
            ...
            if idx == len(self.transformers) - 1:
                ...
                break
            if self.down == 1:
                next_size = self.patch_size[idx] if idx == len(self.transformers) - 1 else self.num_patches[idx+2]
                x, res_faces, center, adjdict = online_process(x, res_faces, adjdict, self.num_patches[idx + 1], center, next_size)
                res_faces_list.append(res_faces)
        return x, feats, res_faces_list