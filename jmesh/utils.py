...
def to_mesh_tensor(meshes):
    if 'faces' in meshes.keys():
        mf = jt.int32(meshes['faces'])
    else:
        mf = None
    meshtensor = MeshTensor(mf, 
                      jt.float32(meshes['feats']), 
                      jt.int32(meshes['Fs']))
    for mesh_key in meshes.keys():
        if mesh_key == "faces" or mesh_key == "feats" or mesh_key == "Fs": continue
        meshtensor._cache[mesh_key] = meshes[mesh_key]
    return meshtensor
...
def save_patches(V, F, res_faces, patch_num, output_name):
    if not output_name.endswith(".ply"):
        output_name += ".ply"
    mesh = Trimesh(vertices=V, faces=F)
    patch_color = np.random.uniform(0,254,(patch_num, 3))
